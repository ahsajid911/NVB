...
export class TogetherProvider implements AIProvider {
  ...
  async chat(messages: ChatMessage[], model: string, temperature: number, maxTokens: number, apiKey: string): Promise<ProviderResponse> {
    ...
    try {
      ...
      if (!response.ok) { const err = await response.text().catch(() => ""); throw new Error(`Together API error ${response.status}: ${(err.length > 500 ? err.substring(0, 500) + "..." : err)}`); }
      const data: any = await response.json();
      const content = data.choices?.[0]?.message?.content;
      if (!content) throw new Error("Together API returned empty response");
    ...
    } catch (err: any) { if (err.name === "AbortError") throw new Error("Together API request timed out (30s)"); throw err; } finally { clearTimeout(timeout); }
  }
