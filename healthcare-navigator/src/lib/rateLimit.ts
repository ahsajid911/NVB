...
export function rateLimit(key: string, opts: RateLimitOptions): RateLimitResult {
  if (upstashAvailable) {
    // Upstash key uses only the action name; IP is handled at the enforceRateLimit level
    // by prepending it to the key, so Upstash sees "action:IP" as the key
    const limiter = getUpstashLimiter("global", opts.limit, opts.windowMs);
    // Upstash Ratelimit expects a plain string key — we pass the full key (action:ip)
    // The result is async, but we wrap it synchronously below in enforceRateLimit
    // For the sync API, we fall back to in-memory and let enforceRateLimit handle Upstash
