...
export class NimProvider implements AIProvider {
  ...
  async chat(messages: ChatMessage[], model: string, temperature: number, maxTokens: number, apiKey: string): Promise<ProviderResponse> {
    if (!apiKey) throw new Error("NIM API key is required");
    ...
    try {
      ...
      if (!response.ok) { const err = await response.text().catch(() => ""); throw new Error(`NIM API error ${response.status}: ${(err.length > 500 ? err.substring(0, 500) + "..." : err)}`); }
    ...
    } catch (err: any) { if (err.name === "AbortError") throw new Error("NIM API request timed out (30s)"); throw err; } finally { clearTimeout(timeout); }
  }
