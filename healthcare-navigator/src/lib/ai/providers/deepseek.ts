...
export class DeepSeekProvider implements AIProvider {
  ...
  async chat(messages: ChatMessage[], model: string, temperature: number, maxTokens: number, apiKey: string): Promise<ProviderResponse> {
    ...
    try {
      ...
      if (!response.ok) { const err = await response.text().catch(() => ""); throw new Error(`DeepSeek API error ${response.status}: ${(err.length > 500 ? err.substring(0, 500) + "..." : err)}`); }
    ...
    } catch (err: any) { if (err.name === "AbortError") throw new Error("DeepSeek API request timed out (30s)"); throw err; } finally { clearTimeout(timeout); }
  }
