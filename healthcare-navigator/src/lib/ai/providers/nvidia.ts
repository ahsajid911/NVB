  ...
  async chat(messages: ChatMessage[], model: string, temperature: number, maxTokens: number, apiKey: string): Promise<ProviderResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`NVIDIA API error ${response.status}: ${(err.length > 500 ? err.substring(0, 500) + "..." : err)}`);
      }

      const data: any = await response.json();
      const content = data.choices?.[0]?.message?.content;
      if (!content) throw new Error("NVIDIA API returned empty response");
      ...
      if (err.name === "AbortError") throw new Error("NVIDIA API request timed out (30s)");
      throw err;
    } finally {
