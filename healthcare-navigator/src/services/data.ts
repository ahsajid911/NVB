...
/**
 * Deterministic pseudo-rating derived from a doctor's id, so the value is
 * stable across requests (important for SEO/SSR and UX). Previously this used
 * Math.random(), producing a different number on every render.
 */
function stableRating(id: string): number {
  ...
  // Map to 4.0 – 4.9
  return Math.round((4.0 + (hash % 100) / 100) * 10) / 10;
}
...
export function getDoctorWithRelations(doctorId: string): DoctorWithRelations | null {
  ...
  return {
    ...
    average_rating: stableRating(doctor.id),
    review_count: stableReviewCount(doctor.id),
  };
}
