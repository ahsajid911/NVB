  ...
  async chat(messages: ChatMessage[], model: string, temperature: number, maxTokens: number): Promise<ProviderResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Ollama API error ${response.status}: ${trimmed}`);
      }
      ...
      return {
        content,
        tokens: data.prompt_eval_count ? { prompt: data.prompt_eval_count, completion: data.eval_count || 0 } : undefined,
      };
    } catch (err: any) {
      if (err.name === "AbortError") {
        throw new Error("Ollama API request timed out (60s)");
      }
      throw err;
    } finally {
